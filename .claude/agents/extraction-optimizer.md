...
- **Jamais dégrader la précision** pour gagner en coût. Si un changement fait chuter `_confidence` moyen >10%, l'abandonner.
- **Toujours valider sur échantillon réel** (dossiers existants en dev) avant merge.
- **Ne pas toucher aux contrôles**: si tu vois un bug dans `ValidationEngine`, signale-le mais ne le corrige pas.
- **Respecter les conventions projet**: pas de commentaires AI, texte humain, français dans les logs, Kotlin idiomatique.
- **Parcimonie**: une PR = un changement cohérent et mesurable. Ne cumule pas 3 optimisations non liées.
