...
/**
 * Categorisation par contexte document : sert le besoin metier (un controleur
 * raisonne par document a la fois — la fiche fiscale, la facture, l'OP — pas
 * par concept technique). Ordre des entrees = ordre d'affichage dans la sidebar.
 */
export type RuleDocCategory =
  | 'dossier'
  | 'ficheFiscale'
  | 'facture'
  | 'bcContrat'
...
export const DOC_CATEGORIES: { key: RuleDocCategory; label: string }[] = [
  { key: 'dossier', label: 'Dossier (completude)' },
  { key: 'ficheFiscale', label: 'Fiche fiscale' },
  { key: 'facture', label: 'Facture' },
  { key: 'bcContrat', label: 'Bon de commande / Contrat' },
  { key: 'factureVsBc', label: 'Facture vs Bon de commande / Contrat' },
  { key: 'op', label: 'Ordre de paiement' },
  { key: 'attestationRib', label: 'Attestation RIB' },
  { key: 'fournisseur', label: 'Coherence fournisseur (transverse)' },
  { key: 'autocontrole', label: 'Autocontrole (humain)' },
  { key: 'custom', label: 'Regles personnalisees (IA)' },
]

export const DOC_CATEGORY_LABEL: Record<RuleDocCategory, string> = DOC_CATEGORIES
  .reduce((acc, c) => { acc[c.key] = c.label; return acc }, {} as Record<RuleDocCategory, string>)

export const ALL_RULES: (ValidationRule & { docCategory?: RuleDocCategory })[] = [
  // 1. Dossier (vue d'ensemble)
  { code: 'R20', label: 'Completude du dossier', desc: 'Verifie que toutes les pieces obligatoires sont presentes',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'dossier',
  ...
  // 2. Facture (intra-document)
  { code: 'R16', label: 'Verification arithmetique HT+TVA=TTC', desc: 'Verifie que HT + TVA = TTC sur la facture',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'facture',
  ...
  // 3. Facture vs Bon de commande / Contrat (cross-doc)
  { code: 'R01', label: 'Concordance montant TTC', desc: 'Compare le TTC de la facture avec le BC',
    category: 'system', appliesToBC: true, appliesToContractuel: false, docCategory: 'factureVsBc',
  ...
  { code: 'R01g', label: 'Matching ligne par ligne facture ↔ BC/contrat', desc: 'Apparie chaque ligne de facture a une ligne du BC (ou a la grille tarifaire du contrat) via le code article ou la similarite du libelle, puis compare quantite, prix unitaire HT et montant HT. Non conforme si une ligne facture n\'a pas de correspondance ou si qte / PU / total divergent au-dela de la tolerance.',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'factureVsBc',
  ...
  // 5. Bon de commande / Contrat (intra-document)
  { code: 'R15', label: 'Grille tarifaire x duree', desc: 'Verifie que la somme des prix mensuels de l\'avenant x nombre de mois = HT facture',
    category: 'system', appliesToBC: false, appliesToContractuel: true, docCategory: 'bcContrat',
  ...
  // 6. References citees dans l'OP (ancrees sur l'OP)
  ...
  // 7. Fiche fiscale (attestation = reference d'identite + validite reglementaire)
  { code: 'R09', label: 'Coherence ICE', desc: 'Verifie que l\'ICE du fournisseur est identique entre facture et attestation fiscale',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'ficheFiscale',
  ...
  { code: 'R11', label: 'Coherence RIB', desc: 'Verifie que le RIB de la facture correspond a celui de l\'OP',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'attestationRib',
  ...
  // 9. Coherence fournisseur — transverse (touche 6 documents differents)
  { code: 'R14', label: 'Coherence fournisseur', desc: 'Compare le nom du fournisseur entre la facture, le bon de commande, l\'ordre de paiement (beneficiaire), le tableau de controle, la checklist (prestataire) et la raison sociale de l\'attestation fiscale. Conforme si tous les noms sont identiques apres normalisation.',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'fournisseur',
  ...
  // 10. Autocontrole humain (CCF-EN-04 + tableau de controle)
  { code: 'R12', label: 'Checklist autocontrole', desc: 'Verifie que tous les points de la checklist sont valides',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'autocontrole',
  ...
  // Chronologie globale BC/contrat -> facture -> OP : ancree sur le couple
  // facture vs BC/Contrat (point amont). R17b (Facture -> OP) est cote backend.
  ...
  { code: 'R18b', label: 'OP couvert par l\'attestation fiscale', desc: 'Verifie que la date d\'emission de l\'OP est anterieure a l\'expiration de l\'attestation fiscale (dateEdition + 6 mois). Empeche un paiement dont la couverture fiscale a expire avant l\'OP.',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'ficheFiscale',
    formula: 'ordrePaiement.dateEmission ≤ attestation.dateEdition + 6 mois',
    method: 'Compare la date d\'emission de l\'OP avec la date d\'expiration calculee de l\'attestation. Non conforme si l\'OP est emis apres l\'expiration : la couverture fiscale du paiement n\'est pas garantie.',
    fields: ['dateEdition', 'dateEmission'],
  },
  ...
  { code: 'R23', label: 'Regularite fiscale du fournisseur', desc: 'Verifie la case cochee sur l\'attestation. Conforme si la case "N\'a pas de dette fiscale exigible" est cochee (estEnRegle=true). Non conforme si la case "N\'est pas en regle quant aux obligations suivantes" est cochee. Avertissement si l\'extraction est ambigue (relecture humaine).',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'ficheFiscale',
    formula: 'attestation.estEnRegle === true',
    method: 'Lit le champ booleen estEnRegle extrait par Claude a partir des cases a cocher du formulaire DGI. true = conforme, false = non conforme, null = avertissement (case ambigue, validation humaine).',
    fields: ['estEnRegle'],
  },
  ...
  { code: 'CK01', label: 'Concordance facture / modalites contractuelles / livrables',
    ...
    category: 'checklist', appliesToBC: true, appliesToContractuel: true, docCategory: 'autocontrole',
