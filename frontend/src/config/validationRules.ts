...
export const ALL_RULES: (ValidationRule & { docCategory?: RuleDocCategory })[] = [
  ...
  { code: 'R16c', label: 'Somme des lignes = HT facture', desc: 'Additionne les montants HT de toutes les lignes de la facture et verifie que le total est egal au montant HT imprime en pied de facture. Si la somme correspond au TTC ou a HT × (1 + taux TVA legal), bascule en AVERTISSEMENT (lignes probablement extraites en TTC car la facture ne contient pas de colonne HT distincte).',
    category: 'system', appliesToBC: true, appliesToContractuel: true, docCategory: 'facture',
    formula: 'Σ facture.lignes[i].montantTotalHT ≈ facture.montantHT (tol 5%)',
    method: 'Somme les montantTotalHT de toutes les lignes extraites et compare au montantHT de pied de facture. Detecte d\'abord les lignes manquees par l\'OCR (vrai NON_CONFORME), puis ecarte les faux positifs lies a une extraction TTC->HT (somme ≈ TTC, ou ratio ≈ 1+taux TVA legal) en degradant le verdict en AVERTISSEMENT.',
    fields: ['lignes.montantTotalHT', 'montantHT', 'montantTTC'],
  },
