...
import {
  ...
  CheckCircle, Pencil, Send, X, Plus, SkipForward
} from 'lucide-react'
...
export default function Finalize() {
  ...
  useEffect(() => {
    ...
    getDossier(id).then(d => {
      setDossier(d)
      // Deduplicate + pre-fill from validation results
      const pts: ControlPoint[] = []

      // Standard 10 TC points
      ...
      // Map validation results to standard points
      for (const desc of standardPoints) {
        ...
        const obs = match
          ...
          : 'NA'
        pts.push({ description: desc, observation: obs, commentaire: match?.detail || '', skip: false })
      }
    ...
    <div>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h2 style={{ marginBottom: 0 }}>
            ...
            <span style={{ fontWeight: 500, fontSize: 11, color: 'var(--ink-40)', marginLeft: 8, textTransform: 'none', letterSpacing: 0 }}>
              {conformeCount}/{activeCount} conformes
            </span>
          </h2>
          <button className="btn btn-secondary btn-sm"
            onClick={() => setPoints(prev => [...prev, { description: '', observation: 'NA', commentaire: '', skip: false }])}>
            <Plus size={11} /> Point
          </button>
        </div>
        ...
        <table className="data-table" style={{ fontSize: 12 }}>
          ...
          <tbody>
            {points.map((pt, i) => (
              <tr key={i} style={{ opacity: pt.skip ? 0.4 : 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
          <div>
            ...
            <div style={{ fontSize: 10, color: 'var(--ink-30)', marginTop: 6, fontFamily: 'var(--font-mono)' }}>
              Date : {new Date().toLocaleDateString('fr-FR')} | Signature sauvegardee localement
            </div>
          </div>
          <div>
