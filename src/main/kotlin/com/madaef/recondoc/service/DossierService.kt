...
class DossierService(
    ...
    private val extractedEntityPersistenceService: com.madaef.recondoc.service.dossier.ExtractedEntityPersistenceService,
    ...
    /**
     * Persiste l'entite typee correspondant aux donnees extraites :
     * - Types standards (FACTURE, BON_COMMANDE, ...) -> delegue a
     *   [ExtractedEntityPersistenceService] qui les mappe sur leur entite JPA.
     * - Types d'engagement (MARCHE, BON_COMMANDE_CADRE, CONTRAT_CADRE) ->
     *   cree/met a jour l'Engagement et rattache le dossier (specifique
     *   a DossierService car le dossier lui-meme est modifie).
     *
     * Le try/catch global garantit qu'un echec de persistance ne casse pas le
     * pipeline d'extraction : le document reste en base avec donneesExtraites,
     * meme si l'entite typee n'a pas pu etre creee.
     */
    private fun saveExtractedEntity(dossier: DossierPaiement, doc: Document, type: TypeDocument, data: Map<String, Any?>) {
        try {
            val handled = extractedEntityPersistenceService.savePrimary(dossier, doc, type, data)
            if (!handled) {
                when (type) {
                    TypeDocument.MARCHE,
    ...
    fun buildFullResponse(dossier: DossierPaiement): DossierResponse {
        return DossierResponse(
            ...
            facture = dossier.factures.distinctBy { it.id ?: it }.firstOrNull()?.let { factureToMap(it) },
            factures = dossier.factures.distinctBy { it.id ?: it }.map { factureToMap(it) },
